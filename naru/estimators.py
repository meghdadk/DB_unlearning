...
class ProgressiveSampling(CardEst):
    ...
    def _sample_n(self,
        ...
        for i in range(ncols):
            ...
            if op is not None:
                ...
                valid_i = OPS[op](reference_columns[natural_idx].all_distinct_values,
                                  vals[natural_idx]).astype(np.float32,
                                                            copy=False)
                
                #this part has beed added
                #to support deletion.
                reduced_index = [1 if element in columns[natural_idx].all_distinct_values else 0 for element in reference_columns[natural_idx].all_distinct_values]
                reduced_index = np.array(reduced_index)
                valid_i = valid_i * reduced_index
    ...
    def Query(self, columns, operators, vals, reference_table, reference_columns):
        # Massages queries into natural order.
        columns, _, _ = FillInUnqueriedColumns(
            self.table, columns, operators, vals)


        reference_columns, operators, vals = FillInUnqueriedColumns(
            reference_table, reference_columns, operators, vals)
