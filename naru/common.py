...
class TableDataset(data.Dataset):
    ...
    def __init__(self, table, base_table=None):
        ...
        self.tuples_np = np.stack(
            [self.Discretize(c, c_base) for c, c_base in zip(self.table.Columns(), base_table.Columns())], axis=1)
        self.tuples = torch.as_tensor(
            self.tuples_np.astype(np.float32, copy=False))
        print('done, took {:.1f}s'.format(time.time() - s))

    def Discretize(self, col, base_col):
        ...
        """
        return Discretize(col, base_col)
...
def Discretize(col, base_col, data=None):
    ...
    if isnan.any():
        ...
        dvs = base_col.all_distinct_values
        bin_ids = pd.Categorical(data, categories=dvs).codes
