...
class MADE(nn.Module):
    ...
    def nll(self, logits, data):
        ...
        return nll
    def kd_loss(self, logits1, logits2):
        """Calculates -log p(data), given logits (the conditionals).
        """

        l = nn.MSELoss(reduction="none")
        kd = torch.zeros(logits1.size()[0], device=logits1.device)
        for i in range(self.nin):
            logits1_i = self.logits_for_col(i, logits1)
            logits2_i = self.logits_for_col(i, logits2)

            s = torch.multinomial(torch.softmax(logits2_i, -1), 1)

            #s = torch.softmax(logits2_i/1, -1)
            kd += F.cross_entropy(logits1_i, s.long().view(-1,), reduction='none')

            #mse = torch.mean(l(logits2_i, logits1_i),dim=1)
            #kd += mse
